...
description: Measures interest in a topic from Wikipedia pageviews and compares language editions. Use for product questions such as "is interest in X growing", "how much can we trust that", "which language should we launch in next". Not for sales, prices or countries.
---
...
- `--langs`: Wikipedia editions to measure, e.g. `uk`, `cs,uk`, `en,de,uk`.
...
| user asks | command |
|---|---|
| compare topic X in two languages over two years | `research --topic "X" --lang uk --langs cs,uk --years 2` |
| is interest in X growing in uk, can we trust it | `research --topic "X" --lang uk --langs uk` |
| compare several languages, short report, recommendation | `research --topic "X" --lang uk --langs en,de,uk` |
...
- `status: needs_choice`: `summary` lists the candidate meanings and asks which one. When the user picks, rerun with `--qid <qid>` instead of `--topic`.
